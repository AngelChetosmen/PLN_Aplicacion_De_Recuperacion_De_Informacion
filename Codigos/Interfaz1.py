...
class PoemAnalyzerApp:
    def __init__(self, master):
        ...
        # Cargar corpus crudo
        self.label_raw_corpus = tk.Label(master, text="Cargar Corpus Crudo:")
        self.label_raw_corpus.pack()
        self.raw_corpus_path = tk.Entry(master, width=50)
        self.raw_corpus_path.pack()
        self.button_load_raw = tk.Button(master, text="Cargar", command=self.load_raw_corpus)
        self.button_load_raw.pack()
        ...
        self.button_analyze = tk.Button(master, text="Analizar", command=self.analyze_poem)
        self.button_analyze.pack()

        # Área para mostrar resultados
        self.results_area = scrolledtext.ScrolledText(master, width=60, height=10)
    ...
    def analyze_poem(self):
        ...
        if not normalized_file_path:
            ...
            normalize_and_save_corpus(raw_file_path, output_file_path)
        ...
        if input_poem:
            input_vector = vectorizer.transform([input_poem])
            cosine_similarities = calculate_cosine_similarity(tfidf_matrix)
            poem_similarities = cosine_similarities[-1]  # Última fila corresponde al poema ingresado
            
            # Obtener los índices de los 10 poemas más similares
            similar_indices = poem_similarities.argsort()[-11:-1][::-1]  # Top 10
            
            results_text = "Top 10 Poemas Similares:\n"
            for index in similar_indices:
                results_text += f"{df['normalized_title'][index]} - Similitud: {poem_similarities[index]:.4f}\n"
            
            # Mostrar resultados
            self.results_area.delete("1.0", tk.END)  # Limpiar área de resultados
